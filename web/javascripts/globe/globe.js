  ...
  var PI_HALF = Math.PI / 2;
	...
	function clearView(){
		for(var i = 0; i < scene_array.length; i++){
			renderer.removeObject( scene_array[i],scene );	
  ...
  addData = function(data, opt_name) {
    ...
    if (this._baseGeometry === undefined) {
      ...
      for (i = 0; i < data.length; i += 3) {
        ...
        color.setHSV( ( 0.6 - ( (0.1) * 0.5 ) ), 1.0, 1.0 );
        addPoint(lat, lng, size/1000, color, this._baseGeometry);
      }
    }
    ...
    for (var i = 0; i < data.length; i += 3) {
      ...
      color.setHSV( ( 0.6 - ( 1.0 * 0.5 ) ), 1.0, 1.0 );
      size = size*200;
...
function plotData(data) {
    ...
    for (i = 0; i < data.length; i += 3) {
	      ...
	      var size = data[i + 2];
		color = new THREE.Color();
		color.setHSV( ( 0.6 - ( 0.4 * 0.5 ) ), 1.0, 1.0 );
		addPoint( lat, lng, 0.4 * 200, color  );
	}
